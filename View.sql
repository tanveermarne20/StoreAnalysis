...
--01.CREATE A VIEW AND DISPLAY WHICH STATE HAS MAXIMUM PUSRCHASE POWER
CREATE OR REPLACE STORE_VIEW_1
  AS
  SELECT SHIP_STATE,SHIP_CITY,SUM(AMOUNT) AS SUMM_POWER
  FROM STORE
  GROUP BY SHIP_STATE,CITY
  ORDER BY MAXIMUM_AMOUNT DESC;
...
CREATE USER STORE_CLIENT IDENTIFIED BY root;
GRANT CREATE SESSION TO STORE_CLIENT;
GRANT SELECT ON STORE_VIEW_1 TO STORE_CLIENT;
...
--2.CREATE A VIEW TO DISPLAY WHICH AGE_GROUP HAVE PURCHASED A LOT
CREATE OR REPLACE VIEW STORE_VIEW_2
  AS
  SELECT AGE_GROUP,SUM(AMOUNT) AS PURCHASE_POWER
  FROM STORE
...
SELECT *
  FROM STORE_VIEW_2;
GRANT SELECT ON STORE_VIEW_2 TO STORE_CLIENT;
...
CREATE OR REPLACE VIEW CHECK_STATUS_AMOUNT_VIEW 
  AS
  SELECT STATUS,COUNT(ORDER_ID) AS TOTAL_NUMBER_OF_ORDERS
  FROM STORE
  GROUP BY STATUS
...
CREATE OR REPLACE VIEW AVG_AHE_VIEW
  FROM STORE
  SELECT GENDER,TRUNC(AVG(AGE))AS AVG_AGE
  FROM STORE
  GROUP BY GENDER;
