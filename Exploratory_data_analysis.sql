...
SELECT GENDER,COUNT(*) AS TOTAL_ORDERS_RECIEVED
  FROM STORE
  GROUP BY GENDER;
...
--WHICH STATE HAS GENERATED HIGHEST AMOUNT OF PROFIT WHERE STATUS IS DELIVERED
SELECT SHIP_STATE,STATUS,SUM(AMOUNT) AS STATE_PROFIT,DENSE_RANK() OVER()
  FROM STORE
...
ORDER BY TOTAL_DELIVERED_PROFIT DESC;
  ...
  SELECT MEDIA,SUM(AMOUNT) AS TOTTAL_AMOUNT_BY_MEDIA
  FROM STORE
  WHERE UPPER(STATUS)='DELIVERED'
  GROUP BY MEDIA
  ORDER BY TOTAL_AMOUNT_BY_MEDIA
...
SELECT MEDIA,AGE_GROUP,CATEGORY,COUNT(*) AS CNT
FROM STORRE
WHERE UPPER(AGE_GROUP)='TEENAGER' AND UPPER(MEDIA)='AMAZON'
GROUP BY MEDIA,AGE_GROUP,CATEGORY
ORDER BY CNT DESC;
